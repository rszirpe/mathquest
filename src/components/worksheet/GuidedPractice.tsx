...
import { MasteryAssessment } from '@/components/mastery/MasteryAssessment'

const GUIDED_COUNT = 10
/** First few problems are a cold-start mastery assessment; the rest are today's free-help practice. */
const ASSESS_COUNT = 3
...
export function GuidedPractice({ topics, difficulty, title, onExit, onDone }: Props) {
  ...
  const [subPhase, setSubPhase] = useState<'assess' | 'practice'>('assess')
  ...
  return (
    <div className="flex min-h-full flex-col bg-gradient-to-b from-violet-400 to-indigo-600 p-4">
      ...
      <div className="mx-auto mb-3 flex w-full max-w-xl items-center justify-between font-display text-sm font-bold text-white/90">
        ...
        {subPhase === 'practice' && (
          <span className="rounded-full bg-white/20 px-3 py-1">
            Practice {idx - ASSESS_COUNT + 1} / {problems.length - ASSESS_COUNT}
          </span>
        )}
      </div>
      ...
      {subPhase === 'assess' ? (
        <MasteryAssessment
          problems={problems.slice(0, ASSESS_COUNT)}
          onAllDone={() => {
            setIdx(ASSESS_COUNT)
            setSubPhase('practice')
          }}
        />
      ) : (
        <>
